...
class BUMDesaController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'tahun_berdiri' => 'required|integer|min:1000|max:' . date('Y'),
            'tahun_laporan' => 'required|integer|min:1000|max:' . date('Y'),
        ]);
        if ($validator->fails()) {
            return redirect()->back()
                ->withErrors(['error' => 'Periksa kembali Tahun Laporan dan Tahun Berdirinya BUMDesa !'])
                ->withInput();
        } else {
    ...
    {
        return Excel::download(new BumdesaExport, "Bumdesa.xlsx");
    }
}
